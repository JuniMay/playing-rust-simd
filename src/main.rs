...
fn matmul_simd(a: &[f32], b: &[f32], c: &mut [f32], m: usize, n: usize, k: usize) {
    for i in 0..m {
        for l in 0..k {
            ...
            for _ in 0..num_simd {
                let c_ij = f32x64::from_slice(&c_in[j..j + simd_width]);
                let b_lj = f32x64::from_slice(&b_ln[j..j + simd_width]);
                let result = c_ij + b_lj * f32x64::splat(a_il);
                result.store_select(&mut c_in[j..j + simd_width], Mask::splat(true));
                j += simd_width;
            }
...
mod tests {
    ...
    fn test_matmul() {
        ...
        for i in 0..m {
            for j in 0..n {
                assert!((c_naive[i * n + j] - c_loop_interchange[i * n + j]).abs() < 1e-6);
                assert!((c_naive[i * n + j] - c_iterator[i * n + j]).abs() < 1e-6);
                assert!((c_naive[i * n + j] - c_simd[i * n + j]).abs() < 1e-6);
                assert!((c_naive[i * n + j] - c_parallelized[i * n + j]).abs() < 1e-6);
            }
        }
    }
}
